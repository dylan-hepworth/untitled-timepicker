...
const TimePicker = ({
	...
	useEffect(() => {
        ...
        // If the `defaultTime` prop is provided, default to that value.
		if (defaultTime) {
	...
	const generateTime = (type) => {
        ...
        if (hoursAvailable?.length && type === "hour") {
            ...
            const uniqueHours = [...new Set(hoursAvailable)];

            // Sort the hours in ascending order
            let hours = uniqueHours.sort();
            ...
            return hoursAvailable.map((hour, i) => {
                ...
                return (
                    <li key={i} onClick={clickEvent}>{hour}</li>
                );
            });
        } else {
